...
const setAlarm = () => {
  ...
  const currentmilliseconds =
    (currhours * 60 * 60 + currmins * 60 + currsecs) * 1000;
  ...
  const alarmtimemilliseconds =
    (alarmtimehours * 60 * 60 + alarmtimemins * 60 + alarmtimesecs) * 1000;
...
const HandleDisplay = (alarmList) => {
  ...
  setAlarmlist.innerHTML += alarmList.map((item) => {
    ...
    <div class="current-time-alarm-left" key=item.id>
      <div class="current-time-alarm">&#11044 ${hours1}:${mins1}:${secs1} ${ampm}</div>
    </div>
